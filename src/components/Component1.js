...
const Component1 = () => {
  ...
  const handleMessage = () => {
    setCountArr(prevClick => [...prevClick, prevClick.length+1]);
    setToastView(true)
  }

  return (
    <>
    {!timerToast && <div className="h-screen">
      <button className='bg-blue-400 hover:bg-blue-500 p-2 rounded-lg text-white m-2'
              onClick={handleMessage}
      >Show Toast Messages</button>
    ...
    <div className='absolute bottom-3 right-2/4 translate-x-2/4 bg-slate-200 w-44 h-44 flex flex-col-reverse justify-start content-around overflow-hidden'>
      {countArr.map((item)=>(<Toasts key={item} countDown={toastCounter} click = {item} />))}
    </div>
    }
    </>
  )
